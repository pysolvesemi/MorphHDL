...
| `lib/src/main/scala/spinal/lib/Stream.scala` | `typed-mechanical-propagation` | retains exact StreamFifo branch owners, symbolic storage and formal-helper geometry; retains joint StreamFifoCC depth/payload-width geometry through stable native Mem write roles; and uses an exact-width, event-safe zero carrier only in the invalid-depth alternative | none; the ordinary Vec access/decoder, pointer, selected-register and storage behavior plus the native StreamFifoCC CDC, reset, Gray-code and memory algorithms remain authoritative |
