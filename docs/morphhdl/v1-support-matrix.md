...
| CDC | Native StreamFifoCC | v1 | Increment 57a carries typed power-of-two depth through the authoritative native dual-clock Gray-pointer FIFO, both `BufferCC` synchronizers, the tagged dual-clock RAM and the existing cross-clock Stream helpers. Clock/reset topology and CDC metadata remain static, and illegal depth specializations fail closed outside the FIFO algorithm. |
