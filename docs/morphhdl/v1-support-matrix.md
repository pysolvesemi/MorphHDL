...
| CDC | Native StreamFifoCC | v1 | Increment 57a carries typed power-of-two depth through the authoritative native dual-clock Gray-pointer FIFO, both `BufferCC` synchronizers, the tagged dual-clock RAM and the existing cross-clock Stream helpers. Joint payload-width/depth equivalence is qualified only when Increment 57b is checked and merged: its finite matrix is `WIDTH` `{1, 5, 8, 32}` by `DEPTH` `{2, 4, 8, 16}`, both reset topologies and both clock-ratio directions. That matrix does not claim universal quantification over arbitrary positive `WIDTH`. Clock/reset topology and CDC metadata remain static, and illegal depth specializations fail closed outside the FIFO algorithm. |
