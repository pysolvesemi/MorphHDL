...
| Libraries | AXI4/AXI4-Lite/AXI-Stream subset | v1 | Only blocks required by DisplayController, plus Increment 57's representative aligned single-address `Axi4SlaveFactory` register-map proof through the generic typed `BusSlaveFactory` surface. Symbolic ranges, masks and bus-family-specific decoders remain out of scope. |
