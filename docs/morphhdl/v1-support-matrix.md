...
| Aggregates | Bundle/Vec internal intent | v1 | Increment 30 preserves native symbolic leaves through `cloneOf`, `HardType`, Bundle and static Vec construction, then flattens them deterministically for Verilog; symbolic Vec length remains deferred |
...
| Libraries | Bits/UInt/SInt and core operators | v1 | Increment 30 retains direct symbolic widths on ordinary `Bits`, `UInt` and `SInt` leaves through cloning and data shapes; derived widths and broader operators remain Increment 31 work |
| Libraries | Stream, Flow and Counter | v1 | Increment 30 preserves symbolic Bundle payload shapes through ordinary Stream and Flow construction and direct leaf connections; Increment 25's Counter and Increment 28's atomic m2s-pipe remain compatibility oracles until generic library lowering |
