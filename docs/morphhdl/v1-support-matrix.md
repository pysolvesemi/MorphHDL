...
| Libraries | Stream, Flow and Counter | v1 | Increment 30 preserves symbolic Bundle payload shapes through ordinary Stream and Flow construction and direct leaf connections. Increment 53f migrates representative native Stream/Flow propagation and the authoritative binary Counter through shared typed geometry adapters while keeping literal overloads concrete-authoritative. |
| Libraries | Synchronous FIFO | v1 | Increment 27 implements the atomic single-clock ready/valid contract; Increments 53e and 53f carry exact typed depth and formal-helper inspection through the unchanged native `spinal.lib.StreamFifo` storage/control algorithms at depths 1, 3, 5 and 8. |
| Libraries | Stream m2s pipeline | v1 | Increment 28 implements the atomic capacity-one registered ready/valid stage matching default `Stream.m2sPipe()`; Increment 53f preserves its native concrete behavior while typed Stream/Flow payload geometry and representative FIFO users remain parameterized. |
