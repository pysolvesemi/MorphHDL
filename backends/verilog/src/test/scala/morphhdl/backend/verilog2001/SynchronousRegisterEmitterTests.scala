...
import morphhdl.paramrtl.ModuleItem.{ContinuousAssign, SynchronousRegister}
...
class SynchronousRegisterEmitterTests extends AnyFunSuite {
  test("emits strict Verilog-2001 output reg and canonical synchronous-reset register") {
    val expected =
      """module SyncRegister (
        |  input  wire [0:0] clk,
        ...
        |);
        |
        |  always @(posedge clk) begin : p_register
        |    if (reset == 1'b1) begin
        |      data_out <= {8{1'b0}};
        |    end else begin
        |      data_out <= data_in;
  ...
  test("sorts ports deterministically without changing process roles") {
    assert(emit(registerDesign()) == emit(registerDesign(reversePorts = true)))
  }

  test("keeps continuous outputs as wires in modules without a register process") {
  ...
  test("rejects reserved process labels before emission") {
    ...
    val process = top.items.head.asInstanceOf[SynchronousRegister]
  ...
  test("retains portable signed-32 checks for register width parameters") {
