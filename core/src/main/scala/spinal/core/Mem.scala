...
object Mem {
  ...
  /** Create a RAM
    ...
    */
  def apply[T <: Data](wordType: HardType[T], wordCount: Int): Mem[T] = new Mem[T](wordType, wordCount)
...
//  override def opName: String = "Mem"
  ...
  private[core] def parameterizedPortAddressWidth(aspectRatio: Int): Option[ElabInt] =
    ParameterizedMemory.portAddressWidthOf(this, aspectRatio)

  /** Audited typed witness consumed only by ordinary native port APIs. */
  private[core] def parameterizedPortAddressWidthWitness(aspectRatio: Int): Option[Int] =
...
class MemReadAsync extends MemPortStatement with WidthProvider  with ContextUser with Expression{
  ...
  def getAddressWidth = mem.parameterizedPortAddressWidthWitness(aspectRatio).getOrElse(log2Up(getWordsCount))
  ...
  override def normalizeInputs: Unit = {
    ...
    val addressReq = mem.parameterizedPortAddressWidthWitness(aspectRatio).getOrElse(mem.addressWidth + log2Up(aspectRatio))
    ...
    if(address.getWidth != addressReq) {
      PendingError(s"Address used to read $mem doesn't match the required width, ${address.getWidth} bits in place of $addressReq bits\n${this.getScalaLocationLong}")
      return
    }
  }
...
class MemWrite() extends MemPortStatement with WidthProvider{
  ...
  override def normalizeInputs: Unit = {
    ...
    if(address.getWidth != addressReq) {
      PendingError(s"Address used to write $mem doesn't match the required width, ${address.getWidth} bits in place of $addressReq bits\n${this.getScalaLocationLong}")
      return
    }
  }
