...
class UInt
    ...
    with Num[UInt]
    with MinMaxProvider
    with DataPrimitives[UInt]
    with BaseTypePrimitives[UInt]
    with BitwiseOp[UInt] {
  ...
  def fixTo(section: Range.Inclusive, roundType: RoundType): UInt = {
    ...
    class fixTo(width: Int, section: Range.Inclusive, roundType: RoundType) extends Component {
  ...
  /** 2'Complement
    ...
    */
  def twoComplement(enable: Bool, plusOneEnable: Bool = null): SInt = ((enable ## Mux(enable, ~this, this)).asUInt + U(
...
//  def twoComplementUInt(enable: Bool): UInt = ((Mux(enable, ~this, this)) + enable.asUInt)
  ...
  override def resize(width: ElabInt): this.type = {
    ...
    if (width.isConcrete) resize(witness)
      ...
      ParameterizedWidth
        .attachResize(resize(witness), width)
  ...
  override def apply(offset: Int, bitCount: BitCount): this.type =
    newExtract(offset + bitCount.value - 1, offset, new UIntRangedAccessFixed).setWidth(bitCount.value)
  override def apply(offset: UInt, bitCount: BitCount): this.type =
    newExtract(offset, bitCount.value, new UIntRangedAccessFloating).setWidth(bitCount.value)
  ...
  override def assignFormalRandom(kind: Operator.Formal.RandomExpKind) =
    this.assignFrom(new Operator.Formal.RandomExpUInt(kind, widthOf(this)))
}
