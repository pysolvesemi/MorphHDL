...
def check(root: Path, label: str, rejection: str | None = None) -> dict:
    ...
    if rejection is None:
        if result.returncode or "sealed current oracles and current native audits PASS" not in result.stdout:
...
def main() -> None:
    ...
    cases = (
        ...
        ("later-committed-production", module.COMPLETED_60F, production, True, None),
        ...
        ("original-untracked-production", module.BASE,
         ...
         "qualified 60f must be an ancestor of HEAD"),
        ("changed-sealed-oracle", module.COMPLETED_60F, oracle, True, "sealed oracle/authority/contract changed"),
        ("changed-signed-authority", module.COMPLETED_60F,
         ...
         "sealed oracle/authority/contract changed"),
    ...
    print("PASS: three positive and nine exact negative inherited 60f source-scope cases", flush=True)
