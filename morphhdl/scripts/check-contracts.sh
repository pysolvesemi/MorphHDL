...
require_property backend.canonical_ir MorphHDL-IR-v1
require_property backend.canonical_ir_stage post-parameterization-pre-emission
require_property backend.canonical_ir_profile simple-wire-assignments-v1
require_property backend.canonical_ir_validation required
require_property backend.canonical_ir_publication opt-in-read-only
require_property backend.canonical_ir_optional_passes disabled-until-WA-07
require_property backend.compatibility_ir ParamRTL
require_property backend.compatibility_emitter direct-verilog
require_property backend.single_source_emitter native-spinal-verilog
require_property backend.emitted_text_role serialization-only
require_property implementation.semantic_authority native-algorithms-and-typed-schemas
...
require_property frontend.legacy_dual_factory deprecated-compatibility-mutation-only
require_property frontend.single_source_parameter_report native-elaboration-parameters
require_property frontend.single_source_paramrtl_report deprecated-compatibility-view
...
require_property formal.streamfifocc.width_domain 1-5-8-32
require_property formal.streamfifocc.positive_configurations 64
require_property formal.streamfifocc.payload_mutation width-5-live-counterexample
