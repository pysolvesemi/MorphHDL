...
/**
  * Independent ordinary-SpinalVerilog witness for one concrete native Int
  * register-map offset. The wrapper keeps the same fixed AXI4 ABI as the
  * parameterized MorphHDL top while giving every concrete module a disjoint
  * definition name.
  */
final class NativeAxi4SlaveFactoryConcreteFormalTop(offset: Int)
    extends Component {
  require(Vector(0x010, 0x040, 0x050, 0x070).contains(offset))
  setDefinitionName(s"NativeAxi4SlaveFactoryConcreteFormalTopOffset$offset")
  ...
  val registers = new NativeAxi4SlaveFactoryRegisterBlock(offset)
  registers.setDefinitionName(
    s"NativeAxi4SlaveFactoryConcreteRegisterBlockOffset$offset"
  )
...
class NativeAxi4SlaveFactoryFormalEquivalenceV2Tests extends AnyFunSuite {
  private val Offsets = Vector(0x010, 0x040, 0x050, 0x070)
  private val FormalGateEnvironment =
    "MORPHDL_RUN_AXI4_FACTORY_FORMAL_EQUIVALENCE_V2"
  private val FormalWorkspaceEnvironment =
    "MORPHDL_AXI4_FACTORY_FORMAL_WORKSPACE_V2"
  private val ParameterizedFile = "native_axi4_factory_parameterized_formal_v2.v"
  ...
  test("formal AXI4 witnesses are independent native-Int elaborations") {
    withTemporaryDirectory { directory =>
      validateGeneratedDuts(generateDuts(directory))
    }
  }

  test("one parameterized native AXI4 factory is formally equivalent to all concrete witnesses") {
    ...
    withFormalWorkspace { directory =>
      ...
      Offsets.foreach { offset =>
        val miter = directory.resolve(
          s"native_axi4_factory_equivalence_offset_$offset.v"
        )
        write(miter, equivalenceMiter(offset, mutateCandidateReady = false))
        val config = directory.resolve(
          s"native_axi4_factory_equivalence_offset_$offset.sby"
        )
        write(
          config,
          positiveSby(
      ...
      val mutationMiter = directory.resolve(
        "native_axi4_factory_equivalence_offset_64_mutation.v"
      )
      write(
        mutationMiter,
        equivalenceMiter(mutationOffset, mutateCandidateReady = true)
      )
      val mutationConfig = directory.resolve(
        "native_axi4_factory_equivalence_offset_64_mutation.sby"
      )
      write(
        mutationConfig,
        mutationSby(
  ...
  private def generateDuts(directory: Path): GeneratedDuts = {
    ...
    val concreteByOffset = Offsets.map { selectedOffset =>
      ...
      val file = s"native_axi4_factory_concrete_offset_$selectedOffset.v"
      concreteConfig.netlistFileName = file
      SpinalVerilog(concreteConfig) {
        new NativeAxi4SlaveFactoryConcreteFormalTop(selectedOffset)
      }
  ...
  private def validateGeneratedDuts(generated: GeneratedDuts): Unit = {
    ...
    assert(parameterized.contains("parameter integer TOP_OFFSET = 64"))
    assert(parameterized.contains("module NativeAxi4SlaveFactoryParameterizedTop #("))
    assert(parameterized.contains("module NativeAxi4SlaveFactoryRegisterBlock #("))
    assert(parameterized.contains(".OFFSET(TOP_OFFSET)"))
    assert(
      parameterized.contains("OFFSET + 4") ||
        parameterized.contains("OFFSET+4") ||
        parameterized.contains("(OFFSET + 4)")
    )
    assert(parameterized.contains("io_axi_aw_ready"))
    assert(parameterized.contains("io_axi_r_valid"))
    assert(parameterized.contains("baseRegister"))
    assert(parameterized.contains("nextRegister"))
    assert(parameterized.contains("fixedRegister"))
    assert(containsAddressLiteral(parameterized, 0x080))
    assert(!parameterized.contains("ParamRTL"))

    val parameterizedModules = moduleNames(parameterized).toSet
    val allConcreteModules = generated.concreteByOffset.toVector.flatMap {
      case (offset, path) =>
        ...
        assert(!concrete.contains(".OFFSET("))
        ...
        val expectedTop =
          s"NativeAxi4SlaveFactoryConcreteFormalTopOffset$offset"
        val expectedChild =
          s"NativeAxi4SlaveFactoryConcreteRegisterBlockOffset$offset"
        val actual = moduleNames(concrete).toSet
        assert(actual.contains(expectedTop))
        assert(actual.contains(expectedChild))
        actual
    }.toSet

    assert(
      allConcreteModules.intersect(parameterizedModules).isEmpty,
  ...
  private def prepareDuts(
    ...
    val candidate = directory.resolve(s"morph_axi4_candidate_offset_$offset.il")
    val candidateScript =
      directory.resolve(s"prepare_morph_axi4_candidate_offset_$offset.ys")
    write(
      candidateScript,
      s"""read_verilog -defer ${yosysPath(generated.parameterized)}
         |chparam -set TOP_OFFSET $offset NativeAxi4SlaveFactoryParameterizedTop
    ...
    write(
      ...
      s"""read_verilog -defer ${yosysPath(generated.concreteByOffset(offset))}
         |hierarchy -check -top $concreteTop
         |flatten
         |proc
         |opt_clean
         |check -assert
  ...
  private def equivalenceMiter(
    ...
    val candidateReady =
      if (mutateCandidateReady) "(morph_aw_ready_raw ^ 1'b1)"
      else "morph_aw_ready_raw"
    ...
    s"""module ${miterModule(offset)} (
       ...
       |  wire morph_aw_ready_raw;
       |  wire morph_aw_ready_compared;
       |  wire morph_w_ready;
       |  wire morph_b_valid;
       |  wire [1:0] morph_b_id;
       ...
       |
       |  assign morph_aw_ready_compared = $candidateReady;
       |
       ...
       |    .io_axi_aw_valid(axi_aw_valid),
       |    .io_axi_aw_ready(morph_aw_ready_raw),
       |    .io_axi_aw_payload_addr(axi_aw_addr),
       |    .io_axi_aw_payload_id(axi_aw_id),
       |    .io_axi_aw_payload_region(axi_aw_region),
       |    .io_axi_aw_payload_len(axi_aw_len),
       ...
       |      assume(reset);
       |    if (!$$initstate) begin
       |      assert(concrete_aw_ready == morph_aw_ready_compared);
  ...
  private def candidateFormalTop(offset: Int): String =
    s"MorphNativeAxi4FactoryFormalCandidateOffset$offset"

  private def concreteFormalTop(offset: Int): String =
    s"ConcreteNativeAxi4FactoryFormalReferenceOffset$offset"
  ...
  private def withTemporaryDirectory(body: Path => Unit): Unit = {
    val directory =
      Files.createTempDirectory("morphhdl-native-axi4-formal-v2-")
    try body(directory)
    finally {
      val stream = Files.walk(directory)
      try {
