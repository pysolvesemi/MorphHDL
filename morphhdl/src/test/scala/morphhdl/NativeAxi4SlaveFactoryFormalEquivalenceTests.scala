...
/** Independent ordinary-SpinalVerilog witness for one native Int register-map offset. */
final class NativeConcreteAxi4SlaveFactoryTop(offset: Int) extends Component {
  ...
  registers.setDefinitionName(
    s"NativeConcreteAxi4SlaveFactoryRegisterBlockOffset$offset"
  )
...
class NativeAxi4SlaveFactoryFormalEquivalenceTests extends AnyFunSuite {
  ...
  private def validateGeneratedDuts(generated: GeneratedDuts): Unit = {
    ...
    assert(parameterized.contains("parameter integer OFFSET = 64"))
    assert(
    ...
    assert(
      parameterized.contains(
        "module NativeAxi4SlaveFactoryRegisterBlock #(" 
      )
    )
    assert(parameterized.contains(".OFFSET(TOP_OFFSET)"))
    assert(parameterized.contains("OFFSET"))
    assert(
    ...
    val allConcreteModules =
      generated.concreteByOffset.toVector.flatMap { case (offset, path) =>
        ...
        assert(
          !concrete.contains("parameter integer TOP_OFFSET"),
          s"Concrete offset $offset unexpectedly retained TOP_OFFSET"
        )
        assert(
          !concrete.contains("parameter integer OFFSET"),
          s"Concrete offset $offset unexpectedly retained OFFSET"
        )
        ...
        val expected = Set(
          s"NativeConcreteAxi4SlaveFactoryTopOffset$offset",
          s"NativeConcreteAxi4SlaveFactoryRegisterBlockOffset$offset"
        )
