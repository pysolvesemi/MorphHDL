...
/** Fifteenth public MorphVerilog contract: one asynchronous enabled register. */
private[morphhdl] object AsynchronousEnabledRegisterContractFixture {
  def program(reverseConstructionOrder: Boolean): MorphProgram[Component] =
    MorphProgram(
      concreteWitness = new Component {
        setDefinitionName("AsynchronousEnabledRegister")
