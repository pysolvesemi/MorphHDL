...
/** Fourteenth public MorphVerilog contract: one synchronous enabled register. */
private[morphhdl] object SynchronousEnabledRegisterContractFixture {
  def program(reverseConstructionOrder: Boolean): MorphProgram[Component] =
    MorphProgram(
      concreteWitness = new Component {
        setDefinitionName("SynchronousEnabledRegister")
