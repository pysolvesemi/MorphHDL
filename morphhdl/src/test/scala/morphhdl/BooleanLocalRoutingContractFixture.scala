...
/** Ninth public MorphVerilog contract: one dependency-ordered Boolean local. */
private[morphhdl] object BooleanLocalRoutingContractFixture {
  def program(reverseConstructionOrder: Boolean): MorphProgram[Component] =
    MorphProgram(
      concreteWitness = new Component {
        setDefinitionName("BooleanLocalRouting")
      ...
      parameterizedDesign = {
        ...
        val routeHigh = localParam(
          "ROUTE_HIGH",
          enable && (effectiveWidth >= limit)
        )
        ...
        def emitRoute(): Unit =
          emitInstance(
            ...
            booleanParameterBindings = Vector(parameterBinding("SELECT", routeHigh)),
            portConnections = ordered(
              Vector(
                portConnection("high_in", ref("high_in")),
                portConnection("low_in", ref("low_in")),
        ...
        val top = moduleDef(
          ...
          booleanLocalParameters = Vector(booleanLocalParameter(routeHigh))
        )
