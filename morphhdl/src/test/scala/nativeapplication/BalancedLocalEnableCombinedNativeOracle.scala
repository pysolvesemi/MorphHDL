...
/** Plain-Int native reference. It does not instantiate/import a Morph candidate
  ...
  */
final case class NativeLocalEnableCombinedWord(uw: Int, upw: Int, sw: Int, spw: Int,
...
final class BalancedLocalEnableCombinedNativeOracle(uw: Int, sw: Int, tw: Int,
    inner: Int, count: Int, mode: Int, moduleName: String)
    extends Component {
  ...
  val lanes = Vector.tabulate(count) { index =>
    val lane = NativeLocalEnableCombinedWord(uw, uw, sw, sw, tw, inner)
  ...
  val pipeline = new ClockingArea(ClockDomain(clock = clk, reset = reset,
    ...
    val reduced = lanes.reduceBalancedTree((a: NativeLocalEnableCombinedWord,
        b: NativeLocalEnableCombinedWord) => {
      val us = (a.us +^ bias) +^ b.us
      val up = a.up * b.up
      val ss = (a.ss +^ offset) +^ b.ss
      val sp = a.sp * b.sp
      val result = NativeLocalEnableCombinedWord(us.getBitsWidth, up.getBitsWidth,
        ss.getBitsWidth, sp.getBitsWidth, tw, inner)
      result.us := us; result.up := up; result.ss := ss; result.sp := sp
      result.sat := a.sat +| b.sat
      ...
      result
    }, (value: NativeLocalEnableCombinedWord, _: Int) => {
      val r = cloneOf(value)
      r.us := RegNextWhen(value.us, value.sat(0) ^ value.ss.msb) init U(1)
      r.up := RegNextWhen(value.up, value.sat.msb ^ value.us(0)) init U(1)
      r.ss := RegNextWhen(value.ss, value.sat(0) ^ value.up.msb) init S(-1)
      r.sp := RegNextWhen(value.sp, value.sat.msb ^ value.ss(0)) init S(-1)
      r.sat := RegNextWhen(value.sat, value.us(0) ^ value.sp.msb) init U(0)
      r.samples.vec.zip(value.samples.vec).foreach { case (to, from) =>
        to := RegNextWhen(from, value.sat(0)) init B(0)
      }
      r
    })
