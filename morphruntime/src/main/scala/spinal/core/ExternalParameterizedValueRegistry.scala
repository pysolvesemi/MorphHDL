...
object ExternalParameterizedValueRegistry {
  ...
  def valuesOf(component: Component): Vector[(UInt, ExternalParameterizedValueRecord)] = {
    ...
    val unique = new IdentityHashMap[UInt, ExternalParameterizedValueRecord]()
    values.foreach { case (value, record) =>
      if (!unique.containsKey(value)) unique.put(value, record)
    }
    val entries = ArrayBuffer.empty[(UInt, ExternalParameterizedValueRecord)]
    val iterator = unique.entrySet().iterator()
