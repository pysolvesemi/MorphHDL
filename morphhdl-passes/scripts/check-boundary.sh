...
wa07_dependencies_satisfied() {
  ...
  grep -Eq '^- \[x\] \*\*WA-06[[:space:]]+—' "${roadmap}" && \
    grep -Eq '^- \[x\] \*\*Increment 48[[:space:]]+—' "${pv_roadmap}"
}
...
if [[ ${#violations[@]} -ne 0 ]]; then
  ...
  if [[ "${is_wa07}" == true ]]; then
    printf 'WA-07 MorphHDL-owned handoff paths are allowed only after WA-06 and PV-48 are checked on the target branch.\n' >&2
  else
