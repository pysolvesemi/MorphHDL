...
final class UnnamedWireAliasEliminationPassSpec
    extends AnyFunSuite
    with Matchers {
  ...
  private def run(design: Design): PassResult[Design] =
    UnnamedWireAliasEliminationPass.run(design, enabled)

  test("unnamed elimination is disabled by default") {
    val design = baseDesign()
    val result = UnnamedWireAliasEliminationPass.run(design)
  ...
  test("eligible unnamed alias is removed by exact symbol identity") {
    val design = baseDesign()
  ...
  test("explicit reflected generated and emitted-name conventions are never unnamed candidates") {
    val origins = Vector[NameOrigin](
      NameOrigin.Explicit("_zz_7"),
  ...
  test("unsafe unnamed alias is retained with deterministic rejection evidence") {
    val design = baseDesign(
  ...
  test("component names and source paths do not affect unnamed classification") {
    val first = run(
      baseDesign(
