...
import morphhdl.paramrtl.ModuleItem.{AsynchronousRegister, ContinuousAssign}
...
class AsynchronousRegisterEmitterTests extends AnyFunSuite {
  test("emits strict Verilog-2001 output reg and canonical asynchronous-reset register") {
    val expected =
      """module AsyncRegister (
        |  input  wire [0:0] clk,
    ...
    assert(emit(registerDesign()) == expected)
  }

  test("uses an atomic public parameter as the reset replication width") {
    val verilog = emit(registerDesign(width = ParameterRef("WIDTH"), parameterized = true))

    assert(verilog.contains("parameter integer WIDTH = 8"), verilog)
    assert(verilog.contains("output reg [WIDTH-1:0] data_out"), verilog)
    assert(verilog.contains("data_out <= {WIDTH{1'b0}};"), verilog)
  }

  test("parenthesizes additive and selected reset replication widths") {
    val additive = emit(
      registerDesign(width = Add(ParameterRef("WIDTH"), Literal(1)), parameterized = true)
    )
    assert(additive.contains("output reg [(WIDTH + 1)-1:0] data_out"), additive)
    ...
    assert(
      selected.contains("output reg [((WIDE == 1) ? WIDTH + 1 : 4)-1:0] data_out"),
      selected
    )
    assert(
  ...
  test("sorts ports deterministically without changing asynchronous process roles") {
    assert(emit(registerDesign()) == emit(registerDesign(reversePorts = true)))
  }

  test("keeps continuous outputs as wires in modules without an asynchronous process") {
  ...
  test("rejects reserved process labels before emission") {
    ...
    val process = top.items.head.asInstanceOf[AsynchronousRegister]
  ...
  test("retains portable signed-32 checks for asynchronous register width parameters") {
