...
class SynchronousEnabledRegisterFrontendTests extends AnyFunSuite {
  private def enabled(label: String = "p_sync_enabled_register"): Unit =
  ...
  test("captures one exact synchronous reset enabled-register intent") {
    val items = captureItems { enabled() }
  ...
  test("unions and discharges integer Boolean and local provenance from all inputs") {
    ...
    val module = moduleDef(
      name = "EnabledRegisterProvenance",
  ...
  test("reports undeclared mismatched and foreign provenance at the retained origin") {
    ...
    val missing = intercept[FrontendException] {
      moduleDef("EnabledIdentityMissing", Vector.empty, Vector.empty, publicItems)
    }
    ...
    val foreign = intercept[FrontendException] {
      moduleDef("ClaimedEnabledReuse", Vector.empty, Vector.empty, claimedItems)
    }
    assert(foreign.code == "MORPH-FRONTEND-LOCAL-PARAMETER-FOREIGN")
    assert(foreign.origin == claimed.origin)
    assert(foreign.detail.contains("ClaimedEnabledOwner"))
  }
  ...
  test("rejects null non-ref and invalid identifiers with stable role diagnostics") {
    ...
    def emit(
      ...
      emitSynchronousEnabledRegister(label, clock, reset, enable, value)
    }
    ...
    val invalidTarget = FrontendNode(
      ...
      origin = SourceOrigin("InvalidEnabledTarget.scala", 7)
    )
    ...
    val invalidValue = FrontendNode(
      ...
      origin = SourceOrigin("InvalidEnabledValue.scala", 9)
    )
  ...
  test("defers declaration direction width type and role-separation proof to ParamRTL") {
    val items = captureItems {
      emitSynchronousEnabledRegister(
        "p_deferred",
        ref("same_control"),
    ...
    assert(items.raw.head == SynchronousEnabledRegister(
      "p_deferred",
      Ref("same_control"),
      Ref("same_control"),
      Ref("same_control"),
    ...
    assert(
      moduleDef("DeferredEnabledRoles", Vector.empty, Vector.empty, items).items == items.raw
    )
  }

  test("requires parameterized capture and remains stateless under concrete elaboration") {
    assert(
      intercept[FrontendException](enabled()).code == "MORPH-FRONTEND-EMIT-OUTSIDE-CAPTURE"
    )
    assert(
      intercept[FrontendException] {
        FrontendSession.concrete(enabled())
  ...
  test("failed calls roll back atomically and a successful process remains singular") {
    ...
    val items = captureItems {
      ...
      enabled("p_retry")
      duplicate = intercept[FrontendException](enabled("p_discarded"))
    }
    assert(
      invalid.code == "MORPH-FRONTEND-SYNCHRONOUS-ENABLED-REGISTER-ENABLE-NOT-REF"
    )
    assert(
      duplicate.code == "MORPH-FRONTEND-SYNCHRONOUS-ENABLED-REGISTER-MULTIPLE"
    )
    assert(
      items.raw.map(_.asInstanceOf[SynchronousEnabledRegister].label) == Vector("p_retry")
    )
  }
  ...
  test("mutually excludes continuous assignments and hierarchy in both orders") {
    ...
    captureItems {
      enabled("p_before_continuous")
      continuousAfter = intercept[FrontendException] {
        emitContinuousAssign("extra", ref("data_in"))
      }
    }
    assert(
      continuousAfter.code == "MORPH-FRONTEND-SYNCHRONOUS-ENABLED-REGISTER-MIXED"
    )

    var afterContinuous: FrontendException = null
    captureItems {
      emitContinuousAssign("extra", ref("data_in"))
      afterContinuous = intercept[FrontendException](enabled("p_after_continuous"))
    }
    assert(
      afterContinuous.code == "MORPH-FRONTEND-SYNCHRONOUS-ENABLED-REGISTER-MIXED"
    )

    var instanceAfter: FrontendException = null
    captureItems {
      enabled("p_before_instance")
      instanceAfter = intercept[FrontendException](emitInstance("helper", "Helper"))
    }
    assert(instanceAfter.code == "MORPH-FRONTEND-SYNCHRONOUS-ENABLED-REGISTER-MIXED")

    var afterInstance: FrontendException = null
    captureItems {
      emitInstance("helper", "Helper")
      afterInstance = intercept[FrontendException](enabled("p_after_instance"))
    }
    assert(afterInstance.code == "MORPH-FRONTEND-SYNCHRONOUS-ENABLED-REGISTER-MIXED")
  }
  ...
  test("rejects generate nesting and generate siblings in both orders") {
    ...
    Vector(
      intercept[FrontendException] {
        captureItems { generateIf(condition) { enabled() } otherwise {} }
      },
      intercept[FrontendException] {
        captureItems {
          generateCase(selector).choice(0, "g_zero") { enabled() }.default("g_other") {}
        }
      },
      intercept[FrontendException] {
        captureItems { for (_ <- 0 until count) enabled() }
      }
    ).foreach { error =>
      assert(error.code == "MORPH-FRONTEND-SYNCHRONOUS-ENABLED-REGISTER-NESTED")
    }

    def enabledThenGenerate(generate: => Unit): FrontendException = {
      var error: FrontendException = null
      captureItems {
        enabled()
    ...
    def generateThenEnabled(generate: => Unit): FrontendException = {
      var error: FrontendException = null
      captureItems {
        generate
        error = intercept[FrontendException](enabled())
      }
      error
    }

    Vector(
      enabledThenGenerate(generateIf(condition) {} otherwise {}),
      generateThenEnabled(generateIf(condition) {} otherwise {}),
      enabledThenGenerate(
      ...
      enabledThenGenerate { for (_ <- 0 until count) () },
      generateThenEnabled { for (_ <- 0 until count) () }
    ).foreach { error =>
      assert(error.code == "MORPH-FRONTEND-SYNCHRONOUS-ENABLED-REGISTER-MIXED")
    }
  }
  ...
  test("rejects escaped scope provenance without leaking a process") {
    ...
    assert(items.raw.map(_.asInstanceOf[SynchronousEnabledRegister].label) == Vector("p_retry"))
        ...
        foreign = executor
          .submit(new Callable[FrontendException] {
            override def call(): FrontendException =
              intercept[FrontendException] { enabled("p_foreign_thread") }
          })
          .get(10, TimeUnit.SECONDS)

        enabled("p_owner_thread")
      }
      assert(foreign.code == "MORPH-FRONTEND-EMIT-OUTSIDE-CAPTURE")
      assert(
        items.raw.head.asInstanceOf[SynchronousEnabledRegister].label == "p_owner_thread"
      )
    } finally {
