...
object ParameterForwardingFixtureGenerator {
  def main(args: Array[String]): Unit = {
    val output = args.toVector match {
      case Vector("--output", path) => Paths.get(path)
      case _ =>
        throw new IllegalArgumentException(
          "Usage: ParameterForwardingFixtureGenerator --output <path>"
        )
    }

    val verilog = Verilog2001Emitter.emit(ParameterForwardingFixture.design()) match {
