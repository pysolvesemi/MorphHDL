...
def fail(message):
    print("Yosys asynchronous-register contract: " + message, file=sys.stderr)
    return 1
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Check AsynchronousRegister is one positive-edge flip-flop with "
            "active-high asynchronous reset-to-zero"
        )
    )
    ...
    top = modules.get("AsynchronousRegister")
    if top is None:
        return fail("missing top module AsynchronousRegister")
    ...
    if sequential_cell.get("type") != "$adff":
        return fail(
            "flip-flop type is {}, expected $adff".format(
                sequential_cell.get("type")
            )
        )
    ...
    if unexpected:
        return fail("unexpected cells beside $adff: {}".format(unexpected))

    expected_connections = {
        "ARST": ports["reset"].get("bits", []),
        "CLK": ports["clk"].get("bits", []),
        "D": ports["data_in"].get("bits", []),
        "Q": ports["data_out"].get("bits", []),
    }
    problem = require_connections(
        sequential_name, sequential_cell.get("connections", {}), expected_connections
    )
    if problem is not None:
        return fail(problem)

    parameters = sequential_cell.get("parameters", {})
    for parameter_name, expected in (
        ...
        if problem is not None:
            return fail("{} {}".format(sequential_name, problem))

    print(
        "Yosys AsynchronousRegister is one {}-bit positive-edge register with active-high asynchronous reset-to-zero".format(
            args.width
        )
    )
    return 0
