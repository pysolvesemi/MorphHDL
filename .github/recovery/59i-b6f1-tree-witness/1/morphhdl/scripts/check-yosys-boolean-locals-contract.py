...
def fail(message):
    print("Yosys Boolean-locals contract: " + message, file=sys.stderr)
    return 1
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Check the exact BooleanLocals hierarchy selected through the "
            "mixed integer/Boolean local-parameter dependency chain"
        )
    )
    ...
    parser.add_argument(
        ...
        required=True,
        choices=("high", "low"),
        help="expected selected BooleanLocalRoute generate-if branch",
    )
    ...
    top = modules.get("BooleanLocals")
    if top is None:
        return fail("missing top module BooleanLocals")
    ...
    problem = require_ports("BooleanLocals", top_ports, expected_top_ports)
    ...
    print(
        "Yosys BooleanLocals selects {} through {} with exact bindings".format(
