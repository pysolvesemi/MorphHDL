...
class FakeApi:
    def __init__(self, output):
        ...
        self.missing_failure_job = False
    ...
    def collection(self, path, key, **query):
        ...
        rows = e['jobs'][:-1] if self.missing_failure_job else e['jobs']
        return [{'id': row['job'], 'run_id': e['run'], 'head_sha': e['head'], 'conclusion': 'failure',
                 'steps': [{'name': row['step'], 'conclusion': 'failure'}]} for row in rows]
...
class DispatchSafety(unittest.TestCase):
    ...
    def test_exact_four_and_durable_intent_before_each_post(self):
        self.execute()
    ...
    def test_every_failed_lane_job_is_required(self):
        self.api.missing_failure_job = True
        with self.assertRaisesRegex(RuntimeError, 'failed boundary jobs missing'):
            self.execute()
        self.assertEqual(self.api.posts, [])
    ...
    def test_api_rejects_source_writes_and_unrelated_dispatches(self):
        ...
        for path, method, payload in (
            ...
            ('/actions/workflows/332279971/dispatches', 'POST', {'ref': MANIFEST['branch']}),
