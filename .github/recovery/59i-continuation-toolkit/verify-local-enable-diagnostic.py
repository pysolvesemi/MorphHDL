#!/usr/bin/env python3
"""Independently inspect downloaded cb093b5a diagnostic artifacts without HDL execution.
...
Examples:
  python3 -B verify-local-enable-diagnostic.py --mode success \
    ...
    --extract-root verified-cb093 --output cb093-verification.json
...
HEAD = 'cb093b5aa235760f2c3149f4327324976aa0495b'
TREE = 'd207a118d2ef0260efe2388c7df0b8d5954c2d04'
...
PINNED_CHECKERS = {
    MAIN_CHECKER: '3f07a3892066f0c174610875fd8bfa608496c2eac9f32c0c1234f4236d1b477c',
...
def source_identity(root, repo, payload):
    ...
    require(receipt['source_env'] == '59i-local-enable-source' and
            receipt['output_env'] == '59i-local-enable-evidence', 'unexpected controller relative paths')
...
def strict_and_formal(row, directory, main, selected=False, mutation=False):
    ...
    require('synth -top miter\ncheck -assert\nstat\n' in synthesis, 'synthesis check script differs')
    file(directory, 'synthesis.log')
    file(directory, 'miter.v')
    ...
    if selected or mutation:
        ...
        require('sat -seq 18 -set-def-inputs -set-at 1 reset ' in script and
                '-prove bad 0 -prove-skip 1 -timeout 120 ' in script and
                'async2sync\ndffunmap\ncheck -assert\n' in script and
                '-set-init-zero' not in script, 'formal constraints differ')
        ...
        if mutation:
            ...
            require(main.SAT_PASS in log and main.SAT_FAIL not in log and script.rstrip().endswith('-verify'),
                    'bounded formal proof did not pass')


def main_hardware(root, paths, main):
    rtl = root / 'hardware-A'
    ...
    for case, row in zip(manifest['cases'], receipt['results']):
        ...
        strict_and_formal(row, directory, main, tuple(case[k] for k in ('uw', 'sw', 'bw')) == (5, 7, 3))
    ...
    strict_and_formal(normalized, output / 'normalized-unmutated', main, True)
    ...
    for label, row in zip(('enable-identity', 'reset-value'), receipt['mutations']):
        ...
        strict_and_formal(row, directory, main, True, True)
    ...
    require(main.PASS + ': 192 native comparisons; 2 semantic mutations rejected' in
            file(root, 'hardware-checks.log').read_text(), 'missing complete main hardware log')
...
def combined_hardware(root, paths, combined):
    ...
    require(combined.PASS + ': 24 native comparisons; one emitted root-enable mutant rejected' in
            file(root, 'combined-checks.log').read_text(), 'missing complete combined hardware log')
    return dict(native_cases=6, comparisons=24, cycles=320, normalized_baselines=1, emitted_mutations=1,
                formal='not-run')


def deterministic_rtl(root, require_postcheck=True):
    ...
    if require_postcheck or (root / 'unchanged-rtl.log').exists():
        unchanged = read_json(file(root, 'unchanged-rtl.log'))
...
def main_cli():
    ...
    parser.add_argument('--controller', type=Path, default=HERE / 'controller-development-v3')
