...
DISPATCHER_PATH = Path(os.environ.get('MORPHDL_59I_DISPATCHER',
    str(ROOT / 'full-ci-controller/dispatch.py'))).resolve()
...
def step_expected(condition, matrix):
    ...
    if condition == "${{ !cancelled() && steps.generate.outcome == 'success' }}":
...
def review_requirements(workflow):
    ...
    special = {
      ...
      'increment-60f-equivalence-closure.yml': [
        ...
        'Match every suite/case identity against sealed increment-59i-regression-inventory.json:2270 cases/227 suites per Scala, explicit zero failures/errors/skips. Compare final inventories across Scala; old2239/226 artifacts are insufficient.',
...
def collect(args):
    ...
    require(plan['schema'] == 1 and plan['repository'] == REPO and plan['target'] == TARGET
            and plan['feature'] == FEATURE and len(plan['workflows']) == 52, 'wrong evidence plan')
    ...
    print(json.dumps(dict(seal=args.seal_sha, execution_passed=sum(row['execution_pass'] for row in summaries),
        workflows=52, artifact_indexes_complete=sum(row['artifact_index_complete'] for row in summaries),
