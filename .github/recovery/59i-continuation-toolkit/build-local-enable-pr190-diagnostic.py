#!/usr/bin/env python3
"""Build an exact unsealed PR190 integration diagnostic; never publish it."""
import argparse
import ast
import base64
import hashlib
...
PRESERVED_SEAL = '58fb59773a2deebba0251b5b626c19a22453f0a4'
PRESERVED_SEAL_TREE = '5ae0ef04c0dc730e49cbb499f2eca6d65b4bf60f'
INTEGRATED_TARGET = '4b8a86e25f5a1a3f0cb4c37dc537a8dd8aa7b097'
INTEGRATED_TARGET_TREE = 'ebe59eecbc8f550d265e78c717fb093603329055'
INTEGRATION_COMMON = 'e0e9f1d7089d3aa513677a2b94c63eb4a7a7791d'
...
def build(root, source, output, templates):
    ...
    require(git(root, 'show', '-s', '--format=%P', source).decode().split() ==
            [PRESERVED_SEAL, INTEGRATED_TARGET], 'development source must be the exact ordered target merge')
    for ref, tree in ((PRESERVED_SEAL, PRESERVED_SEAL_TREE), (INTEGRATED_TARGET, INTEGRATED_TARGET_TREE)):
        require(git(root, 'rev-parse', ref + '^{tree}').decode().strip() == tree, 'fixed parent tree changed')
    require(git(root, 'merge-base', '--all', PRESERVED_SEAL, INTEGRATED_TARGET).decode().splitlines() ==
            [INTEGRATION_COMMON], 'target merge common base changed')
    require(git(root, 'show', source + ':' + CONTRACT) ==
            git(root, 'show', PRESERVED_SEAL + ':' + CONTRACT), 'development changed preserved seal bytes')
    require(not git(root, 'diff', BASE, source, '--', 'docs/morphhdl/parameterized-verilog-todo.md'),
            'unqualified merge changed completion TODO')
    ...
    payload = dict(base=BASE, base_tree=BASE_TREE, head=source,
        ...
        preserved_seal=PRESERVED_SEAL, preserved_seal_tree=PRESERVED_SEAL_TREE,
        integrated_target=INTEGRATED_TARGET, integrated_target_tree=INTEGRATED_TARGET_TREE,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--templates', type=Path, default=Path(__file__).parent / 'controller-development-pr190-templates')
