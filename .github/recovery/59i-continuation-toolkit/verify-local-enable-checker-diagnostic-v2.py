...
def priority_obligation(root, paths, main, config, successful):
    ...
    original_work = original_root / 'priority-obligation'
    sources = [original_root / 'hardware-A' / entry['file'] for entry in (case, candidate)] + [original_work / 'miter.v']
    top, script, log_path = (v4.file(work, name) for name in ('miter.v', 'equivalence.ys', 'formal.log'))
    require(top.read_text() == main.miter(case, [candidate], ('unsigned', 0)), 'priority output-bit miter differs')
    require(script.read_text() == main.formal_setup(sources) +
        'write_json ' + main.quoted(original_work / 'normalized-netlist.json') + '\n' +
        'sat -seq 18 -set-def-inputs -set-at 1 reset 1 -set-at 1 enable 1 '
        '-prove bad 0 -prove-skip 1 -timeout 120 -verify\n', 'priority proof script/constraints differ')
    log = log_path.read_text()
    passed = log.count(main.SAT_PASS) == 1 and main.SAT_FAIL not in log
    netlist = v4.read_json(v4.file(work, 'normalized-netlist.json'))
    require(set(netlist['modules']) == {'miter'}, 'priority normalized netlist top differs')
    if successful:
        require(passed, 'priority output-bit proof did not pass')
        ...
        require(receipt == dict(scope='previous-timeout-only', checker_source=v4.HEAD,
            ...
            result='pass-18-steps', rtl_sha256={entry['file']: v4.file_digest(v4.file(rtl, entry['file']))
                for entry in (case, candidate)}, miter_sha256=v4.file_digest(top),
            script_sha256=v4.file_digest(script), log_sha256=v4.file_digest(log_path),
    ...
    return dict(case=case['id'], candidate=candidate['module'], field='unsigned', bit=0,
                passed=passed, cells=len(netlist['modules']['miter']['cells']),
                normalized_netlist_sha256=v4.file_digest(v4.file(work, 'normalized-netlist.json')))
