...
def workflow_run(row, head=module.HEAD, status='queued', conclusion=None):
    return {
        ...
        'html_url': ('https://github.com/pysolvesemi/MorphHDL/actions/runs/' +
                     str(row['id'] + 1000)),
    }
...
class FakeApi:
    ...
    def request(self, path, method='GET', payload=None):
        ...
        if path == '/pulls/191':
            ...
            return {
                ...
                'head': {
                    ...
                    'ref': MANIFEST['branch'],
                    'sha': 'changed' if moved else module.HEAD,
                },
                'base': {
        ...
        if path.startswith('/git/ref/heads/'):
            expected = (module.HEAD if path.endswith(MANIFEST['branch'])
                        else MANIFEST['target'])
            return {'object': {'sha': expected}}
        if path.startswith('/actions/runs/'):
            row = next(row for row in MANIFEST['workflows']
                       if path.endswith(str(row['failed_run'])))
            run = workflow_run(row, row['failed_head'], 'completed',
    ...
    def collection(self, path, key, **query):
        ...
        row = next(row for row in MANIFEST['workflows'] if row['failed_run'] == rid)
        head = 'wrong' if self.bad_job_head else row['failed_head']
        return [{'id': rid + 1, 'run_id': rid, 'head_sha': head,
                 'conclusion': 'failure'}]
...
class DispatchSafety(unittest.TestCase):
    ...
    def test_exact_allowlist_and_durable_intent_before_each_post(self):
        self.execute()
        ...
        self.assertTrue(all('run_id' in record for record in self.journal['records']))

    def test_existing_success_and_active_runs_are_reused(self):
        self.api.runs = [
            workflow_run(row,
                         status='completed' if index % 2 else 'in_progress',
                         conclusion='success' if index % 2 else None)
            for index, row in enumerate(MANIFEST['workflows'])
        ]
        self.execute()
        self.assertEqual(self.api.posts, [])
        self.assertTrue(all(record['action'] == 'reuse'
                            for record in self.journal['records']))

    def test_all_existing_successes_are_reused_without_dispatch(self):
        self.api.runs = [workflow_run(row, status='completed', conclusion='success')
    ...
    def test_any_new_head_failure_stops_the_entire_batch_before_writes(self):
        self.api.runs = [workflow_run(MANIFEST['workflows'][-1],
    ...
    def test_skipped_or_cancelled_is_not_reusable_success(self):
        for conclusion in ('skipped', 'cancelled', 'timed_out', 'neutral'):
            with self.subTest(conclusion=conclusion):
                self.api.runs = [workflow_run(MANIFEST['workflows'][0],
                                               status='completed',
                                               conclusion=conclusion)]
    ...
    def test_uncertain_post_is_not_retried(self):
        ...
        self.assertEqual(len(self.api.posts), 1)
        value = json.loads(self.output.read_text())
        self.assertEqual(value['records'][-1]['action'], 'dispatch-uncertain')

    def test_original_failure_and_per_workflow_job_source_are_required(self):
        self.api.bad_failure = True
        with self.assertRaisesRegex(RuntimeError, 'Historical failed-run'):
            self.execute()
        self.assertEqual(self.api.posts, [])
        self.api.bad_failure = False
        self.api.bad_job_head = True
