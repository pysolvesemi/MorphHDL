...
class FakeApi:
    ...
    def request(self, path, method='GET', payload=None):
        ...
        if path.startswith('/git/ref/heads/'):
            return {'object': {'sha': module.HEAD if path.endswith(MANIFEST['branch'])
    ...
    def collection(self, path, key, **query):
        ...
        return [{'id': rid + 1, 'run_id': rid, 'head_sha': MANIFEST['failed_head'],
                 'conclusion': 'failure'}]
...
class DispatchSafety(unittest.TestCase):
    ...
    def test_exact_one_and_durable_intent_before_post(self):
        self.execute()
        ...
        self.assertIn('run_id', self.journal['records'][0])

    def test_existing_active_run_is_reused(self):
        self.api.runs = [workflow_run(MANIFEST['workflows'][0], status='in_progress')]
        self.execute()
        self.assertEqual(self.api.posts, [])
        self.assertEqual(self.journal['records'][0]['action'], 'reuse')

    def test_existing_success_is_reused(self):
        self.api.runs = [workflow_run(MANIFEST['workflows'][0], status='completed', conclusion='success')]
        self.execute()
        self.assertEqual(self.api.posts, [])
        self.assertEqual(self.journal['records'][0]['action'], 'reuse')

    def test_new_head_failure_stops_before_write(self):
        self.api.runs = [workflow_run(MANIFEST['workflows'][0], status='completed', conclusion='failure')]
        with self.assertRaisesRegex(RuntimeError, 'needs diagnosis'):
            self.execute()
        self.assertEqual(self.api.posts, [])

    def test_duplicate_runs_stop_before_write(self):
    ...
    def test_skipped_or_cancelled_is_not_reused(self):
    ...
    def test_ref_movement_stops_after_request(self):
    ...
    def test_uncertain_post_is_not_retried(self):
        ...
        with self.assertRaises(TimeoutError):
            self.execute()
        self.assertEqual(len(self.api.posts), 1)
        self.assertEqual(json.loads(self.output.read_text())['records'][-1]['action'],
                         'dispatch-uncertain')
    ...
    def test_api_rejects_source_writes_and_unrelated_dispatches(self):
        ...
        for path, method, payload in (
            ...
            ('/actions/workflows/332476711/dispatches', 'POST', {'ref': MANIFEST['branch']}),
            ('/pulls/191', 'PATCH', {}),
        ):
