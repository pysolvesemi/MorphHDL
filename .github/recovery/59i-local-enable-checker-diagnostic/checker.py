...
def priority_worker(root, out):
    ...
    # Call exactly the main checker's positive-proof helper. The priority gate
    # must not acquire a second proof formulation or a smaller state/bound model.
    proof_result = checker.prove_bit_equivalence(rtl, work, case, candidate, 'unsigned', 0)
    require(proof_result['candidate'] == candidate['module'] and
            proof_result['field'] == 'unsigned' and proof_result['bit'] == 0 and
            proof_result['result'] == 'pass-18-steps' and
            proof_result['proved_steps'] == list(range(2, 19)),
            'previously timed-out output-bit obligation did not prove all required steps')
    proof_files = ('miter.v', 'state-extraction.ys', 'state-extraction.log',
                   'normalized-netlist.json', 'strengthened-netlist.json',
                   'state-correspondence.json', 'equivalence.ys', 'formal.log', 'finite-proof.json')
    require(all((work / name).is_file() and not (work / name).is_symlink() for name in proof_files),
            'priority proof is missing required model, correspondence or finite-proof evidence')
    write(receipt, dict(scope='previous-timeout-only', checker_source=HEAD,
        ...
        miter_sha256=digest(work / 'miter.v'), script_sha256=digest(work / 'equivalence.ys'),
        log_sha256=digest(work / 'formal.log'),
        normalized_netlist_sha256=digest(work / 'normalized-netlist.json'),
        proof=proof_result, proof_files={name: digest(work / name) for name in proof_files},
