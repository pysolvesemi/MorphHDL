#!/usr/bin/env python3
"""One-shot exact-head PR191 baseline dispatch; never writes source or refs."""
import argparse
import hashlib
import json
import os
...
class Api:
    ...
    def request(self, path, method='GET', payload=None):
        ...
        if method == 'POST':
            ...
            require(match and int(match[1]) == WORKFLOW_ID, 'Non-allowlisted write')
            require(payload == {'ref': self.manifest['branch']}, 'Unexpected dispatch payload')
        else:
            require(method == 'GET' and payload is None,
                    'Only reads and the exact dispatch are allowed')
    ...
    def collection(self, path, key, **query):
        ...
        while True:
            ...
            part = self.request(path + '?' + suffix)[key]
            rows.extend(part)
            if len(part) < 100:
...
def authenticate_failure(api, manifest):
    ...
    require(run['head_sha'] == row['failed_head'] and
            ...
            run['workflow_id'] == WORKFLOW_ID and
...
def existing_run(api, manifest):
    runs = api.collection('/actions/runs', 'workflow_runs', head_sha=HEAD)
    matches = [run for run in runs if run['workflow_id'] == WORKFLOW_ID]
    require(len(matches) <= 1, 'Multiple exact-head runs; reconcile before dispatch')
    if not matches:
        return None, runs
    run = matches[0]
    row = manifest['workflow']
    require(run['head_sha'] == HEAD and run['head_branch'] == manifest['branch'] and
            run['event'] == 'workflow_dispatch' and
            run['path'].split('@')[0] == row['path'],
            'Run source or workflow identity mismatch')
    require(run['status'] in ACTIVE or
            (run['status'] == 'completed' and run['conclusion'] == 'success'),
            'Exact-head terminal failure or skip needs diagnosis, not dispatch')
    return run, runs
...
def dispatch(api, manifest, journal, output):
    ...
    current, runs = existing_run(api, manifest)
    row = manifest['workflow']
    record = {'workflow_id': WORKFLOW_ID, 'workflow_path': row['path']}
    ...
    write_journal(output, journal)
    refs_guard(api, manifest)
    current, _ = existing_run(api, manifest)
    if current:
        record.update(action='reuse', run_id=current['id'], url=current['html_url'])
        journal['phase'] = 'requests-complete'
        write_journal(output, journal)
        return
    try:
        api.request('/actions/workflows/' + str(WORKFLOW_ID) + '/dispatches',
...
def main():
    ...
    require(manifest['repository'] == REPOSITORY and manifest['head'] == HEAD and
            manifest['workflow']['id'] == WORKFLOW_ID, 'Manifest target mismatch')
    ...
    require(os.environ.get('GITHUB_EVENT_NAME') == 'push', 'Reviewed push required')
    require(os.environ.get('GITHUB_RUN_ATTEMPT') == '1',
            'Controller retries prohibited; reconcile the journal')
    ...
    require(hashlib.sha256((args.source / row['path']).read_bytes()).hexdigest() ==
            row['sha256'], 'Workflow bytes changed')
    api = Api(manifest)
    run_id = int(os.environ['GITHUB_RUN_ID'])
    controller = api.request('/actions/runs/' + str(run_id))
    require(controller['head_sha'] == os.environ['GITHUB_SHA'] and
            controller['path'].split('@')[0] == manifest['controller_path'],
            'Controller run mismatch')
    prior = api.collection('/actions/runs', 'workflow_runs',
                           branch=manifest['controller_branch'])
    require(not [run for run in prior if run['id'] != run_id and
                 run['path'].split('@')[0] == manifest['controller_path']],
            'Prior controller run exists; never relaunch it')
    args.output.mkdir(parents=True, exist_ok=True)
    journal = {'head': HEAD, 'tree': manifest['tree'],
               'controller_run_id': run_id,
               'controller_sha': os.environ['GITHUB_SHA'],
               'phase': 'preflight', 'records': []}
