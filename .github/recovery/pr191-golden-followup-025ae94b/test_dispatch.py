...
class DispatchSafety(unittest.TestCase):
    ...
    def test_exact_three_and_durable_intent_before_each_post(self):
        self.execute()
        ...
        self.assertTrue(all('run_id' in row for row in self.journal['records']))
    ...
    def test_existing_success_and_active_runs_are_reused(self):
        ...
        self.assertTrue(all(row['action'] == 'reuse' for row in self.journal['records']))

    def test_partial_existing_set_dispatches_only_missing(self):
        self.api.runs = [workflow_run(MANIFEST['workflows'][0],
                                      status='completed', conclusion='success')]
        self.execute()
        self.assertEqual(set(self.api.posts),
                         {row['id'] for row in MANIFEST['workflows'][1:]})

    def test_any_new_head_failure_stops_entire_batch_before_writes(self):
