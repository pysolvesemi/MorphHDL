#!/usr/bin/env python3
"""One-shot PR191 failed-workflow dispatch; never writes source or refs.
...
HEAD = 'e26eceae4eec3466133543f1261f5493a631d183'
ALLOWED_IDS = {361097183, 332476711, 332279971, 354645379,
               351553191, 362756881, 357516223, 351327055}
    ...
    def request(self, path, method='GET', payload=None):
        ...
        request = urllib.request.Request(
            ...
            headers={'Authorization': 'Bearer ' + os.environ['GITHUB_TOKEN'],
                     ...
                     'Content-Type': 'application/json', 'User-Agent': 'pr191-targeted-ci'})
...
def authenticate_failures(api, manifest):
    for row in manifest['workflows']:
        ...
        require(run['head_sha'] == manifest['failed_head'] and
                run['workflow_id'] == row['id'] and run['path'].split('@')[0] == row['path'] and
                run['status'] == 'completed' and run['conclusion'] == 'failure',
                'Historical failed-run identity or conclusion changed')
        jobs = api.collection('/actions/runs/' + str(run['id']) + '/jobs', 'jobs', filter='latest')
        require(jobs and any(job['conclusion'] == 'failure' for job in jobs),
                'No substantive failed job supports this target')
        require(all(job['run_id'] == run['id'] and job['head_sha'] == manifest['failed_head']
                    for job in jobs), 'Historical job source mismatch')
...
def main():
    ...
    require(len(manifest['workflows']) == 8 and
