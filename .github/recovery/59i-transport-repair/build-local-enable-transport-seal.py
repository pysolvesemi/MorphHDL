#!/usr/bin/env python3
"""Prepare the reviewed schema-5 transport-fix manifest for one exact committed source.
...
EXPECTED_HELPER = "44cddecca2f07d3ea00b8bcbb1d66d502e5fc1ea564b1ee3762f95479d6c015c"
EXPECTED_PARENT = "9de3fd243a28a6d1e6ba385bc2d746b57e337fc1"
EXPECTED_PARENT_SOURCE = "99aacc05a783b9dc51aaeabd4e1395de385cab28"
EXPECTED_PARENT_MANIFEST = "8748c74e631d0de7bbd57f38eceabdf2b5f93176dad78f3fd31585d2e307dab4"
...
def load_helper(root):
    ...
    require(digest(normalized) == EXPECTED_HELPER, "source helper differs from reviewed schema-5 PR190 integration implementation")
    module = types.ModuleType("reviewed_local_enable_seal_preparation")
    module.__file__ = str(filename)
    exec(compile(raw, str(filename), "exec"), module.__dict__)
    require(module.BASE == EXPECTED_BASE and module.PR190_PARENT == EXPECTED_PARENT and
        module.PR190_PARENT_SOURCE == EXPECTED_PARENT_SOURCE and
        module.PR190_PARENT_MANIFEST == EXPECTED_PARENT_MANIFEST,
...
def line_spans(path, before, after, reason, domain):
    ...
    for operation, a, b, c, d in difflib.SequenceMatcher(
        ...
        if operation != "equal":
            result.append({"id": "pr190-" + domain + ":" + path + ":" + str(len(result) + 1),
                "reason": reason,
    ...
    for path in sorted(paths):
        ...
        if old is not None and all(old[key] == value for key, value in exact_identity.items()):
            ...
            entry = dict(exact_identity,
                reason=(old["reason"] + " " if old else "") + "Committed-head transport continuation: " + rationale,
...
def prepare(root, source, reasons_path):
    ...
    target_commit, common, _ = helper.integration_parameters(5)
    previous_target = previous["target_integration"]
    require(previous_target["target_commit"] == target_commit and
        previous_target["common_base"] == common and target_commit == helper.PR190_TARGET,
        "previous target is not the exact preserved PR190 integration")
    ...
    value = {"schema_version": 5, "predecessor": EXPECTED_BASE,
        ...
        "previous_seal": helper.previous_certificate(5),
        "target_checkpoint": helper.pr190_checkpoint(),
        "target_integration": {"target_commit": target_commit, "common_base": common,
            "target_tree": git(root, "rev-parse", target_commit + "^{tree}").decode().strip(),
            "common_base_tree": git(root, "rev-parse", common + "^{tree}").decode().strip(),
            "files": target_files}}
    helper.validate_contract(value)
    helper.verify_pr190_development_history(root, value)
    # These run the original frozen 9de3 predecessor chain and original PR190 target
    ...
    summary = {"source_commit": source, "source_tree": value["source_tree"],
        ...
        "schema_version": 5, "source_records": len(files), "source_records_preserved": len(reused),
