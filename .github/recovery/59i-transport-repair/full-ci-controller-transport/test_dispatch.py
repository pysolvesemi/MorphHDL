...
ROOT=Path('/workspace/scratch/68d458e0e937/59i-transport-reviewed')
...
class Controls(unittest.TestCase):
    ...
    def test_moved_ref_prevents_every_post(self):
        ...
        with self.assertRaises(RuntimeError):self.execute()
        self.assertEqual(self.api.posts,[])
    def test_stale_pr_base_is_accepted_only_with_exact_live_target(self):
        get=self.api.get
        def stale(path,**query):
            value=get(path,**query)
            if path=='/pulls/177':value['base']['sha']='e0e9f1d7089d3aa513677a2b94c63eb4a7a7791d'
            return value
        with patch.object(self.api,'get',side_effect=stale):self.execute('plan')
        self.assertEqual(self.api.posts,[])
    def test_live_target_movement_is_rejected_even_with_matching_pr_metadata(self):
        get=self.api.get
        def moved(path,**query):
            value=get(path,**query)
            if path=='/git/ref/heads/parameterized-verilog':value['object']['sha']='c'*40
            return value
        with patch.object(self.api,'get',side_effect=moved):
            with self.assertRaises(RuntimeError):self.execute()
        self.assertEqual(self.api.posts,[])
    def test_read_only_plan_never_posts(self):
        self.execute('plan');self.assertEqual(self.api.posts,[])
    def test_successful_local_gate_reused_other52_only(self):
        self.execute();self.assertEqual(len(self.api.posts),52)
