...
def validate_fleet(value):
    ...
    require(len(rows)==53 and sorted(r['workflow'] for r in rows)==list(APPLICABLE),
        'applicable fleet differs from reviewed 53 workflows')
    ...
    require(value['expected_inventory']['test_cases']==2307 and
        value['expected_inventory']['suites']==230 and value['expected_inventory']['local_enable_cases']==31,
...
def prepare(args):
    ...
    print(json.dumps({'manifest':str(args.output),'sha256':sha(args.output.read_bytes()),
        'applicable':53,'excluded':14,'network_requests':0}))
...
def clean_source(root, source, seal):
    ...
    require(certificate['schema_version']==5 and certificate['source_commit']==source and
...
def guard(api, seal):
    ...
    require(pr['state']=='open' and not pr['merged'] and pr['draft'] and
        ...
        pr['base']['ref']=='parameterized-verilog' and
        target['object']['sha']==TARGET,'PR feature/target identity or draft state changed')
    # PR base.sha can lag a target update; the direct branch ref above is
    # authoritative. Exact feature, target, branch names and PR state remain gates.
...
def run_controller(args):
    ...
    try:
        ...
        require(len(manifest['applicable'])==53 and
            sorted(r['workflow'] for r in manifest['applicable'])==list(APPLICABLE),'53-workflow inventory changed')
        ...
        receipt['completed_controller']=True;receipt['applicable_count']=53;receipt['exclusion_count']=14
