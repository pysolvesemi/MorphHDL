...
class SynchronousEnabledRegisterValidatorTests extends AnyFunSuite {
  test("accepts one complete rising-edge synchronous-reset enabled register") {
  ...
  test("accepts exact parameterized packed types and unrelated extra inputs") {
    ...
    val base = registerModule("ParameterizedEnabledRegister", packed = packed)
    val top = base.copy(
  ...
  test("requires resolved unsigned one-bit input clock reset and enable controls") {
    Vector("clock" -> registerModule("MissingClock", clock = "missing"),
      "reset" -> registerModule("MissingReset", reset = "missing"),
      "enable" -> registerModule("MissingEnable", enable = "missing")).foreach { case (_, top) =>
  ...
  test("keeps clock reset enable and data roles distinct") {
  ...
  test("requires the target to be the sole module output") {
  ...
  test("rejects every sibling item and every other runtime process kind") {
  ...
  test("rejects enabled registers nested in every generate region") {
  ...
  test("rejects multiple processes and process label failures deterministically") {
    ...
    val first = base.items.head.asInstanceOf[SynchronousEnabledRegister]
  ...
  test("orders mixed-item diagnostics independently of construction order") {
    val base = registerModule("StableEnabledRegisterDiagnostics")
    val process = base.items.head
    val conflicts = Vector[ModuleItem](
      ContinuousAssign(Ref("data_out"), Ref("data_in")),
      CombinationalIf(
  ...
  private def registerDesign(): Design = {
    val top = registerModule("SyncEnabledRegister")
    Design(top.name, Vector(top))
  }

  private def registerModule(
      name: String,
      label: String = "p_sync_enabled_register",
      clock: String = "clk",
      reset: String = "reset",
