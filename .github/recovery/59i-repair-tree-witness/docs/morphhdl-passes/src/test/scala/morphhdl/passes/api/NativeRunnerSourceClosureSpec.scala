...
/** Native proof runners must not omit dependencies of the shared pipeline. */
final class NativeRunnerSourceClosureSpec extends AnyFunSuite with Matchers {
  ...
  private def hasCompleteSources(script: String, registration: String): Boolean =
    script.split("\n").iterator.map(_.trim)
      .dropWhile(line => line != "sbt -batch \\" && !line.startsWith("sbt -batch "))
      .takeWhile(!_.contains("morph / Test / runMain"))
      .contains(registration)

  test("native runners compile the complete pass workspace before first emission") {
    runners.foreach { case (runner, registration) =>
      withClue(runner + ": shared pipeline dependencies must not be a stale per-file list: ") {
        hasCompleteSources(read(runner), registration) shouldBe true
      }
    }
  }
  ...
  test("source-closure guard rejects missing commented late and partial registration") {
    runners.foreach { case (runner, registration) =>
      ...
      withClue(runner + ": ") {
        hasCompleteSources(original, registration) shouldBe true
        hasCompleteSources(original.replace(registration, ""), registration) shouldBe false
        hasCompleteSources(original.replace(registration, "# " + registration), registration) shouldBe false
        hasCompleteSources(original.replace(registration, "") + "\n" + registration, registration) shouldBe false
        hasCompleteSources(
          original.replace("unmanagedSourceDirectories", "unmanagedSources"), registration
        ) shouldBe false
      }
    }
  }
