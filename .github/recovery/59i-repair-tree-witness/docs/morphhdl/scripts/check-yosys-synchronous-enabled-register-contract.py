...
def fail(message):
    print("Yosys synchronous-enabled-register contract: " + message, file=sys.stderr)
    return 1
...
def check_sdffe(name, cell, ports, width):
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Check SynchronousEnabledRegister is one positive-edge enabled "
            "register with active-high synchronous reset-to-zero"
        )
    )
    ...
    top = modules.get("SynchronousEnabledRegister")
    if top is None:
        return fail("missing top module SynchronousEnabledRegister")
    ...
    if cell.get("type") == "$sdffe":
        ...
        if unexpected:
            return fail("unexpected cells beside $sdffe: {}".format(unexpected))
        problem = check_sdffe(name, cell, ports, args.width)
        if problem is not None:
            return fail(problem)
    else:
        return fail(
            "flip-flop type is {}, expected $sdffe after proc/opt_dff".format(
                cell.get("type")
            )
        )

    print(
        "Yosys SynchronousEnabledRegister is one {}-bit positive-edge active-high enabled register with active-high synchronous reset-to-zero".format(
            args.width
        )
    )
    return 0
