...
Continuation correction (2026-09-19): GitHub's PR response still reports base
e0e9 while the direct target ref is exact4b8a86. The controller now pins TARGET
using the authoritative direct ref and checks the PR's base branch name,
without treating the stale PR base SHA as a second authoritative ref. The
feature ref/head, PR state/draft/merged status and base/head branch checks stay
mandatory. `test_ref_identity.py` accepts that observed stale metadata only
with the exact live target and rejects eight moved-ref/PR-state mutations.
