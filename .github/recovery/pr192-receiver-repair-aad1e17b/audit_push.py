...
repo = Path(os.environ.get('MORPHDL_REPO', root.parents[2]))
...
new = yaml.safe_load((repo / m['controller_path']).read_text())
...
report = {'target': m['target'], 'controller_branch': m['controller_branch'],
          'existing_push_matches': matches, 'only_new_controller_matches': True,
          'workflows': records}
(root / 'push-intersection.json').write_text(json.dumps(report, indent=2) + '\n')
