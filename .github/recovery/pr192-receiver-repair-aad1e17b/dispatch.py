...
HEAD = 'aad1e17b23e58beea985e7ddacbfbea5ab68b84e'
ALLOWED_IDS = {357516223, 362756881, 354645379, 337719848}
...
def authenticate_failure(api, manifest):
    ...
    expected = {row['job']: row['step'] for row in evidence['jobs']}
    actual = {
        job['id']: next((step['name'] for step in job['steps'] if step['conclusion'] == 'failure'), None)
        for job in jobs if job['id'] in expected and job['run_id'] == evidence['run'] and
        job['head_sha'] == evidence['head'] and job['conclusion'] == 'failure'
    }
    require(actual == expected, 'Original failed boundary jobs missing')
...
def main():
    ...
    require(len(manifest['workflows']) == 4 and
