...
for found in re.finditer(r"(?:new\s+)?StreamFifo\s*\(", block):
    ...
    if re.search(r"\bdepth\s*=\s*4\b", arguments) or re.search(
        ...
        call_match = (found.start(), opening, closing, arguments)
        break
if call_match is None:
    fail("static depth-four StreamFifo fixture not found")
call_start, call_open, call_close, call_arguments = call_match
if re.search(r"\bdepth\s*=\s*4\b", call_arguments):
    call_arguments = re.sub(
        r"\bdepth\s*=\s*4\b",
        "depth = symbolicDepth",
        call_arguments,
        count=1,
...
{indent})
'''
replacement_call = "StreamFifo(" + call_arguments + ")"
block = (
    block[:line_start]
    + setup
...
test_text = test_text[:marker_index] + new_marker + test_text[marker_index + len(marker) : outer_open + (len(new_marker) - len(marker)) + 1] + block + test_text[outer_close + (len(new_marker) - len(marker)) :]
