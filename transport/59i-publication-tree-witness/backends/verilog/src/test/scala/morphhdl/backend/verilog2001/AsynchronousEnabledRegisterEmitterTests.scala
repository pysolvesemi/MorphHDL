...
import morphhdl.paramrtl.ModuleItem.{AsynchronousEnabledRegister, ContinuousAssign}
...
class AsynchronousEnabledRegisterEmitterTests extends AnyFunSuite {
  test("emits asynchronous reset priority capture-on-enable and implicit hold") {
    val expected =
      """module AsyncEnabledRegister (
        |  input  wire [0:0] clk,
  ...
  test("uses atomic and precedence-safe reset replication widths") {
    val parameter = emit(registerDesign(width = ParameterRef("WIDTH"), parameterized = true))
    assert(parameter.contains("output reg [WIDTH-1:0] data_out"), parameter)
  ...
  test("sorts ports deterministically without changing asynchronous enable roles") {
    assert(emit(registerDesign()) == emit(registerDesign(reversePorts = true)))
  }

  test("keeps legacy continuous outputs as wires") {
  ...
  test("rejects reserved process labels before emission") {
    ...
    val process = top.items.head.asInstanceOf[AsynchronousEnabledRegister]
  ...
  test("retains portable signed-32 checks for asynchronous enabled-register parameters") {
